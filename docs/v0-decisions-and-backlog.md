...
| D44 | **Changes are reviewed as a tree of files, and a file opens in a floating window inside the app, over the map.** The panel's tabs are Conversation · Changes · Summary. Changes lists all of an experiment's changes, one run's own commit, or what is not committed yet; nothing expands inline. Windows drag, resize, minimise, maximise, tile side by side and stack; at most four; an open file is brought forward, not duplicated. They belong to the selected experiment — switching hides them, returning restores them — and never cover the panel. On a narrow window the file fills the screen. Nothing opens a browser tab or leaves the app. Owner-approved 16 September 2026. | The old view opened the first four files of every change, CONTEXT.md included, and the conversation repeated the same diffs; reaching the last file meant scrolling past the rest. The owner wanted several files side by side as in an editor, and rejected an "open in a new tab" route. Summaries carry counts, never patches, and a file's patch is fetched when opened, so a thousand-file change is still a list; the single-file route only serves paths listed in that change's own summary. Comparing different experiments side by side stays out of scope (B8). |
